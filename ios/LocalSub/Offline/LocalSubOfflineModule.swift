...
@objc(LocalSubOfflineModule)
final class LocalSubOfflineModule: NSObject {
    ...
    Task.detached(priority: .userInitiated) {
      do {
        guard let videoURI = payload["videoURI"] as? String else {
          throw LocalSubOfflineError.invalidPayload("Missing video URI.")
        }
        guard let subtitlesArray = payload["subtitles"] as? [[String: Any]] else {
          throw LocalSubOfflineError.invalidPayload("Missing subtitles.")
        }
...
private extension LocalSubOfflineModule {
  func exportBurnedInVideo(
    ...
    guard let sourceVideoTrack = asset.tracks(withMediaType: .video).first else {
      throw LocalSubOfflineError.videoTrackMissing
    }
    ...
    guard let compositionVideoTrack = composition.addMutableTrack(
      ...
      throw LocalSubOfflineError.exportFailed("Unable to build a composition track.")
    }
    ...
    guard let exportSession = AVAssetExportSession(
      ...
      throw LocalSubOfflineError.exportFailed("Unable to create an export session.")
    }
  ...
  func extractAudio(from asset: AVAsset) async throws -> URL {
    guard asset.tracks(withMediaType: .audio).isEmpty == false else {
      throw LocalSubOfflineError.noAudioTrack
    }
    ...
    guard let exportSession = AVAssetExportSession(
      ...
      throw LocalSubOfflineError.exportFailed("Unable to create an audio export session.")
    }
  ...
  func exportAudioChunk(from asset: AVAsset, timeRange: CMTimeRange) async throws -> URL {
    guard let exportSession = AVAssetExportSession(
      ...
      throw LocalSubOfflineError.exportFailed("Unable to create a chunked audio export session.")
    }
    ...
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
      session.exportAsynchronously {
        ...
        case .failed, .cancelled:
          continuation.resume(
            throwing: session.error ?? LocalSubOfflineError.exportFailed("Export cancelled.")
          )
        default:
          continuation.resume(
            throwing: LocalSubOfflineError.exportFailed("Export stopped unexpectedly.")
          )
        }
      }
    }
  ...
  func detectSpeechLocale(for asset: AVAsset) async throws -> String {
    ...
    guard availableLocales.isEmpty == false else {
      throw LocalSubOfflineError.noSupportedSpeechLocales
    }
    ...
    guard probeRanges.isEmpty == false else {
      throw LocalSubOfflineError.noDetectableSpeechLocale
    }
  ...
  func onDeviceRecognizer(for locale: String) throws -> SFSpeechRecognizer {
    guard let recognizer = SFSpeechRecognizer(locale: Locale(identifier: locale)) else {
      throw LocalSubOfflineError.speechUnavailable
    }

    #if !targetEnvironment(simulator)
    guard recognizer.supportsOnDeviceRecognition else {
      throw LocalSubOfflineError.onDeviceRecognitionUnavailable
    }
    #endif
  ...
  func recognizeSpeech(from asset: AVAsset, locale: String) async throws -> [[String: Any]] {
    guard string(from: SFSpeechRecognizer.authorizationStatus()) == "authorized" else {
      throw LocalSubOfflineError.speechAuthorizationDenied
    }
  ...
  func generateThumbnail(for asset: AVAsset) throws -> URL {
    ...
    guard let data = image.jpegData(compressionQuality: 0.84) else {
      throw LocalSubOfflineError.thumbnailFailed
    }
  ...
  func persistentProjectMediaDirectory() throws -> URL {
    guard let supportDirectory = FileManager.default.urls(
      ...
      throw LocalSubOfflineError.invalidPayload("Unable to open app media storage.")
    }

    let directory = supportDirectory
      .appendingPathComponent("LocalSub", isDirectory: true)
  ...
  func normalizedFileURL(from value: String) throws -> URL {
    ...
    if value.isEmpty {
      throw LocalSubOfflineError.invalidPayload("Missing file path.")
    }
  ...
  func saveVideo(url: URL) async throws -> String {
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<String, Error>) in
      ...
      PHPhotoLibrary.shared().performChanges({
        ...
        guard success else {
          continuation.resume(
            throwing: LocalSubOfflineError.exportFailed("Photos save failed.")
          )
          return
        }
  ...
  private var inactivityReminderIdentifiers: [String] {
    ["localsub-inactivity-1", "localsub-inactivity-2", "localsub-inactivity-3"]
  }
}

private enum LocalSubOfflineError: LocalizedError {
